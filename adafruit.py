...
class AdafruitBuilder:
    def __init__(self, env, frameworkDir, platform, board, variant):
        ...
        self.softdevice_version = board.get(
            "build.softdevice.sd_version", "6.1.1")
        self.bootloader_version = board.get(
            "build.bootloader.version", "0.2.6")
        self.softdevice_name = self.board.get("build.softdevice.sd_name")
        self.board_name = self.board.get(
            "build.bootloader", self.board.get("build.variant"))
    ...
    def add_cppdefines(self):
        self.env.Append(
            CPP_DEFINES=[
                ...
                ("ARDUINO_BSP_VERSION", '\\"%s\\"' % self.bsp_version)
            ]
        )
    ...
    def process_softdevice(self):
        if self.softdevice_name:
            self.env.Append(
                CPPPATH=[
                    join(self.nordicDir, "softdevice", "%s_nrf52_%s_API" % (
                        self.softdevice_name, self.softdevice_version), "include")
                ],
                CPPDEFINES=[
            ...
            hex_path = join(self.frameworkDir, "bootloader",
                            self.board.get("build.variant"))
